...
while True:
    ...
    lines_RGB = cv2.HoughLinesP(gray, 1, np.pi/180, 100,
        ...
        for line in lines_RGB:
            for x1,y1,x2,y2 in line:
                ...
                if (angle > gradient_v_min) and (angle < gradient_v_max):
                    linesRGB_filtered.append(line)    
    
    # Only keep vertical depth lines
    if lines_depth is not None:
        for line in lines_depth:
            for x1,y1,x2,y2 in line:
                ...
                if (angle > gradient_v_min) and (angle < gradient_v_max):
                    linesD_filtered.append(line)       
                    
    
    separation_threshold_pixels = 5
    
    intersections = []
    
    # Iterate through all RGB lines, work out seperation from nearest depth image line
    if linesRGB_filtered is not None and linesD_filtered is not None:
        for line_rgb in linesRGB_filtered:
            ...
            for line_depth in linesD_filtered: 
                # Calculate the gradient
